crossManagerHybridauth.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper crossmanagerhybridauth-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('crossmanagerhybridauth-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('crossmanagerhybridauth-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('crossmanagerhybridauth-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('crossmanagerhybridauth-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    crossManagerHybridauth.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(crossManagerHybridauth.grid.Default, MODx.grid.Grid, {
    ...
    getColumns: function () {
        return [{
            ...
            header: _('crossmanagerhybridauth_actions'),
            dataIndex: 'actions',
    ...
    getSearchField: function (width) {
        return {
            xtype: 'crossmanagerhybridauth-field-search',
            width: width || 250,
            listeners: {
                search: {
                    fn: function (field) {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = crossManagerHybridauth.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('crossmanagerhybridauth-grid-default', crossManagerHybridauth.grid.Default);