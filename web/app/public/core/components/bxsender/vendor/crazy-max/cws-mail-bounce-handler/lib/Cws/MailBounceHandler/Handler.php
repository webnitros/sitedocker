...
class Handler
{
    ...
    {
        return count($this->emlFiles) > 0 ? true : false;
    }
    ...
    private function processMailParsing($token, $content)
    {
        ...
        $hash = $this->getHashMessage($body);
        $cwsMbhMail->setHashQueue($hash);
    ...
    /**
     * Вернет хэш сообщения в базе данных
     */
    public function getHashMessage($body)
    {
        $result = $this->parseBodySectionMachine($body);
    ...
    {
        if (!empty($arHeader)) {
            $pregSubject = 'mail delivery failed|mail failure|failure notice|warning: message|delivery status notif|delivery failure|delivery problem|';
            ...
            $pregSubject .= 'mail failure|undeliverable|warning: could not send message for past 4 hours|';
            $pregSubject .= 'Не удается доставить|Недоставленное сообщение|';
            ...
            if (isset($arHeader['Subject'])) {
                $Subject = mb_decode_mimeheader($arHeader['Subject']);
            }

            if (isset($arHeader['Subject'])
                && (preg_match('#(' . $pregSubject . ').*office#i', $arHeader['Subject']) or ($Subject && preg_match('#(' . $pregSubject . ').*office#i', $Subject)))) {
                return true;
